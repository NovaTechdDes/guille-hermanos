...
export default function StockScreen() {
  ...
  const filteredStock = useMemo(() => {
    ...
    return stock.filter((item) => item.nombre.toLowerCase().includes(buscador.toLowerCase()));
  }, [stock, buscador]);
  ...
  return (
    <SafeAreaView className="flex-1 px-4 bg-neutral-50 dark:bg-neutral-950">
      ...
      <FlatList
        ...
        renderItem={({ item, index }) => <ListStockItem key={item.id_insumo} item={item} isLast={index === (filteredStock?.length ?? 0) - 1} />}
...
const EmptyData = () => (
  <View className="py-20 items-center">
    <Text className="text-neutral-400 dark:text-neutral-500 font-medium">No hay insumos cargados</Text>
  </View>
);
