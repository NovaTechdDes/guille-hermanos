...
export default function DestinoComponent() {
  ...
  const [id_destino, setIdDestino] = useState<string>('');
  ...
  const handlePutDestino = async (nombre: string) => {
    const res = await startPutDestino.mutateAsync({ id_destino, nombre });
    if (res) {
      mensaje('success', 'Destino modificado exitosamente');
      setNombre('');
      setVisible(false);
      setInitialNombre('');
      setIdDestino('');
    } else {
      mensaje('error', 'Error al modificar destino');
    }
  };
  ...
  return (
    <View>
      ...
      />
      <ToastNombre visible={visible} initialName={initialNombre} onConfirm={handlePutDestino} onCancel={() => setVisible(false)} />
    </View>
  );
}
