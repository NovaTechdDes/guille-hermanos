...
export default function ListStockItem({ item, isLast }: Props) {
  ...
  const displayId = item.id_insumo?.toString().slice(0, 8).toUpperCase() || 'N/A';
    ...
    >
      ...
      <View className="flex-1 mr-4">
        <View className="bg-neutral-100 dark:bg-neutral-800 self-start px-2 py-0.5 rounded mb-1">
          <Text className="text-[10px] font-bold text-neutral-400 dark:text-neutral-500 tracking-tighter">ID-{displayId}</Text>
        </View>

        <Text className="text-lg font-bold text-neutral-800 dark:text-neutral-100 leading-tight mb-2">{item.nombre}</Text>
      </View>

      {/* Stock y Navegacion */}
      <View className="flex-row items-center gap-4">
        <View className="items-end">
          <Text className="text-[9px] font-black text-neutral-400 dark:text-neutral-500 uppercase tracking-[2px] mb-0.5">STOCK</Text>
          ...
          <View className="flex-row items-center">
            <View className="bg-secondary/15 dark:bg-secondary/25 px-2 py-0.5 rounded">
              <Text className="text-[10px] font-black text-secondary uppercase tracking-widest">{item.unidad}</Text>
            </View>
          </View>
        </View>
